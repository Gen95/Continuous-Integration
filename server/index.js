...
app.get('/style.css', (req, res) => {
    res.sendFile('style.css', {root: path.join(__dirname, '../html')});
})

app.get('/script.js', (req, res) => {
    res.sendFile('script.js', {root: path.join(__dirname, '../html')});
})
...
function releaseCurrentAgent(agent) {
    for (let i = 0, length = agentList.length; i < length; i++) {
        if (agent && agentList[i].host === agent.host && agentList[i].port === agent.port) {
            agentList[i].isFree = true;
        }
    }
}

function createNewHTML(req) {
...
function buildHtml(req) {
    ...
    ${HtmlPart1}
        ...
        <div style="white-space: pre-wrap">${req.body.result}</div>
    ${HtmlPart2}
    `;
}
